...
const ID = FungiesId.describe('Customer UUID')
const BILLING = z.record(z.string(), z.unknown())
...
export function registerUsers(server: McpServer, client: FungiesClient) {
  ...
  server.registerTool('users_create', {
    ...
    inputSchema: strictShape({
      ...
      billingDetails: BILLING.optional().describe('Optional billing details (name, address, taxId, etc.)'),
    }),
  }, async (args) => {
  ...
  server.registerTool('users_update', {
    ...
    inputSchema: strictShape({
      ...
      billingDetails: BILLING.optional().describe('New billing details object'),
    }),
  }, async ({ id, ...patch }) => {
