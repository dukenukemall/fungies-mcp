...
import { ANN, safely, toolJson } from './shared.js'
...
export function registerWebhooks(server: McpServer, client: FungiesClient) {
  ...
  server.registerTool('webhooks_verify', {
    ...
    inputSchema: {
      body: z.string().describe('Raw HTTP request body as received, byte-for-byte'),
      signature: z.string().describe('Value of the x-fngs-signature header (with or without sha256= prefix)'),
      secret: z.string().describe('Your webhook signing secret from Fungies dashboard'),
    },
